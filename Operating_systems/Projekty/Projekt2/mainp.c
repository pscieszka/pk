...
#define P 30   // liczba  procesow producent   w i konsument   w
#define MAX 30 //rozmiar puli bufor   w
#define MAX2 32// rozmiar pami   ci dzielonej
...
int main()
{
   ...
   if (shmID==-1) {printf("blad pami   ^yci dzielonej\n"); exit(1);}

   kluczs=ftok(".",'C');
   semID=semget(kluczs,1,IPC_CREAT|IPC_EXCL|0666);
   if(semID==-1) {printf("blad semafor      w \n");exit(1);}
